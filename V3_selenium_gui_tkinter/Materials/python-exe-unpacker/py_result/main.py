...
import inspect, ctypes, datetime, requests
...
tk.Button(frm01, text="运行\n倒计\n时间", font=('华文新魏', 19), command=ti_shi).grid(row=4,
  column=2,
  rowspan=3,
  padx=6,
  pady=6)
...
tk.Radiobutton(frm10, text="开启代理ip", variable=popo, value=0, command=kkk).grid(row=3, column=0, padx=10, pady=10)
tk.Radiobutton(frm10, text="开启代理Api", variable=popo, value=1, command=kkk).grid(row=3, column=1, padx=10, pady=10)
            ...
            for a in soup.find_all(class_="fieldset"):
                for c in a.find_all("div", type=True):
                    ...
                    if den == 1:
                        ...
                        f.write(y)
                        f.write("\n")
                        for q in c.find_all("div")[None[:1]]:
                            f.write(q.text)
...
tk.Button(frm02, text="分析题目", font=('华文中宋', 12), command=fangshi1).grid(row=1, column=0, columnspan=1, padx=10, pady=10)
tk.Button(frm02, text="导出函数", font=('华文中宋', 12), command=fangshi2).grid(row=1, column=1, columnspan=1, padx=10, pady=10)
...
tk.Button(frm11, text="保存所有数据", command=All_data, font=('华文中宋', 12)).grid(row=5, column=2, columnspan=1, padx=10,
  pady=10)
tk.Button(frm02, text="清除所有数据", command=qc_sj, font=('华文中宋', 12)).grid(row=3, column=0, columnspan=1, padx=10, pady=10)
tk.Button(frm02, text="导入数据", font=('华文中宋', 12), command=daoru_sj).grid(row=3, column=1, columnspan=1, padx=10, pady=10)
...
tk.Button(frm02, text="刷新日志", font=('华文新魏', 14), command=sxlogtxt).grid(row=3, column=2, columnspan=1, padx=10, pady=10)
...
def wenjuan1(driver, q_num, text):
    try:
        ...
        try:
            jsString = "document.getElementById('q{}').removeAttribute('readonly')".format(q_num)
            driver.execute_script(jsString)
            a = driver.find_element(By.XPATH, '//input[@id="q' + str(q_num) + '"]//label//span').send_keys(random.choice(text))
        except:
            try:
...
def wenjuan2(driver, question_number, text):
    try:
        driver.find_element(By.XPATH, '//textarea[@id="q' + str(question_number) + '"]').send_keys(random.choice(text))
    except Exception:
        try:
            driver.find_element(By.XPATH, '//input[@id="q' + str(question_number) + '"]').send_keys(random.choice(text))
        except Exception:
            traceback.print_exc()
...
def wenjuan6(driver, question_number, number, option_num, weight):
    try:
        p = biil(option_num, weight)[0]
        driver.find_element(By.XPATH, '//tr[@id="drv' + str(question_number) + "_" + str(number) + '"]//td//ul//li[' + str(p) + "]").click()
    except Exception:
        try:
            p = biil(option_num, weight)[0]
            driver.find_element(By.XPATH, '//tr[@id="drv' + str(question_number) + "_" + str(number) + '"]' + "/td[" + str(p) + "]").click()
            return p
        except:
            try:
                p = biil(option_num, weight)[0]
                driver.find_element(By.XPATH, '//div[@id="div' + str(question_number) + '"]//tbody//tr[' + str(number) + "]/td[" + str(p) + "]").click()
            except:
                pass
...
def wenjuan7(driver, question_number, option_num, weight):
    try:
        answers = driver.find_element(By.XPATH, '//div//span[@id="select2-q%s-container"]' % str(question_number))
        answers.click()
        p = biil(option_num, weight)[0]
        time.sleep(1)
...
def wenjuan9(driver, q_num, q, text):
    try:
        driver.find_element(By.XPATH, '//input[@id="q' + str(q_num) + "_" + str(q) + '"]' + "/following-sibling::label[1]//span").send_keys(random.choice(text))
    except:
        try:
            driver.find_element(By.XPATH, '//input[@id="q' + str(q_num) + "_" + str(q - 1) + '"]').send_keys(random.choice(text))
        except:
            try:
                driver.find_element(By.XPATH, '//div[@id="div' + str(q_num) + '"]' + "//label//span").send_keys(random.choice(text))
            except:
                pass
...
def wenjuan12(driver, q_num, q, text):
    try:
        driver.find_element(By.XPATH, '//tr[@id="drv' + str(q_num) + "_" + str(q) + '"]//input').send_keys(random.choice(text))
    except:
        pass
...
def gwenjuan9(driver, q_num, q, text):
    try:
        driver.find_element(By.XPATH, '//input[@id="q' + str(q_num) + "_" + str(q) + '"]' + "/following-sibling::label[1]//span").send_keys(text)
    except:
        try:
            driver.find_element(By.XPATH, '//input[@id="q' + str(q_num) + "_" + str(q - 1) + '"]').send_keys(text)
        except:
            pass
    ...
    try:
        ...
        while ctimes < int(save2[1]):
            ...
            try:
                ...
                driver = None
                try:
                    cm = DriverCacheManager(valid_range=7)
                    myservice = Service(ChromeDriverManager(cache_manager=cm).install())
                    if _os_type_ == "windows":
                    ...
                    traceback.print_exc()
                    messagebox.showerror("驱动错误", traceback.format_exc())
                    break
                ...
                try:
                    ...
                    traceback.print_exc()
                    messagebox.showerror("出错", traceback.format_exc())
                ...
                try:
                    WebDriverWait(driver, 12).until(ECS.presence_of_element_located((By.CLASS_NAME, "submit_tip_color")))
                    ...
                    fileName = os.path.join(BASE_DIR, "日志运行" + str(nx) + ".txt")
                    with open(fileName, "a", encoding="utf-8") as f:
                        logttxet = "第" + str(ctimes) + "次已完成!" + "--" + "开始时间：" + str(stat_time) + "--" + "结束时间：" + str(end_time) + "==" + "当前使用的代理是" + str(prox) + "==" + "运行时间" + str(dat_time) + "秒"
                        f.write(logttxet)
                        f.write("\n")
                        f.write("\n")
                    driver.quit()
                    ...
                    fileName = os.path.join(BASE_DIR, "日志运行" + str(nx) + ".txt")
                    with open(fileName, "a", encoding="utf-8") as f:
                        logttxet = "运行" + str(nx) + "====超时出错第" + str(flag) + "次--" + "开始时间：" + str(stat_time) + "--" + "结束时间：" + str(end_time) + "==" + "当前使用的代理是" + str(prox) + "==" + "运行时间" + str(dat_time) + "秒"
                        f.write(logttxet)
                        f.write("\n")
                        f.write("\n")
                        try:
                    ...
                    if flag > int(save6[2]):
                        fileName = os.path.join(BASE_DIR, "日志信息.txt")
                        with open(fileName, "a", encoding="utf-8") as f:
                            logttxet = "运行" + str(nx) + "超时失败 " + str(flag) + " 次--已经停止"
                            f.write(logttxet)
                            f.write("\n")
                        break
                    driver.quit()
...
tk.Button(frm11, text="一键暂停所有", command=stop_all, font=('华文中宋', 12)).grid(row=5, column=1, columnspan=1, padx=10,
  pady=10)
for i in range(15):
    sxstmp = "\ndef tmpd" + str(i + 1) + "():\n    dx(" + str(i + 1) + ")\ndef tmpf" + str(i + 1) + "():\n    fx(" + str(i + 1) + ")\n    "
    exec(sxstmp)
