...
# Embedded file name: PyInstaller\loader\pyimod02_importers.py
"""
PEP-302 and PEP-451 importers for frozen applications.
"""
import sys, os, pathlib, io, tokenize, _frozen_importlib
...
class PyiFrozenImporter:
    ...
    def get_data(self, path):
        ...
        fullname = path[SYS_PREFIXLEN[:None]]
        if fullname in self.toc:
            return self._pyz_archive.extract(fullname)
        ...
        if path is not None:
            ...
            for p in path:
                if not p.startswith(SYS_PREFIX):
                    ...
                    p = p[SYS_PREFIXLEN[:None]]
                    parts = p.split(os.sep)
                    if not parts:
                        continue
                    parts = parts[0] or parts[1[:None]]
    ...
    def exec_module(self, module):
        ...
        if isinstance(spec.loader_state, PyiFrozenImporterState):
            ...
            if isinstance(spec.loader_state, dict):
                ...
                module_name = spec.origin[SYS_PREFIXLEN[:None]].replace(os.sep, ".")
                if module_name.endswith(".pyc"):
                    module_name = module_name[None[:-4]]
                if module_name.endswith(".__init__"):
                    module_name = module_name[None[:-9]]
            else:
